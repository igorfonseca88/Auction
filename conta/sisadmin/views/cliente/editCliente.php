...
<div id="conteudo">
    ...
    <div class="titulo">
        ...
        <p><span><span>Você está em:</span> <a href="/revistafalaserio/">Principal</a> &raquo; 
                <a href="<?= BASE_URL(); ?>clienteController/">Listagem de clientes</a> &raquo; Editar cadastro de cliente</span>
    </div>

    <div class="formulario">
        <h2>Editar cadastro de cliente</h2>
        <p><? echo $this->session->flashdata('sucesso'); ?></p>
        <form method="post" action="<?= BASE_URL(); ?>clienteController/editCliente">

            <? foreach ($cliente as $row) { ?>
                <input type="hidden" name="idClienteh" id="idClienteh" value="<?= $row->idCliente ?>"/>
                <div class="item">
                    <label>CNPJ/CPF</label><br />
                    <input type="text" name="txtCPF_CNPJ" id="txtCPF_CNPJ" value="<?= $row->cnpj_cpf ?>" class="input"/>
                </div>
